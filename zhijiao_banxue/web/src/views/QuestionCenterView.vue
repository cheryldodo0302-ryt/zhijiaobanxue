...
async function loadStatistics() {
  ...
  try {
    statistics.value = (await api.get(
      ...
      { params: { ...(classId.value ? { class_id: classId.value } : {}), _refresh: Date.now() } },
    )).data
  ...
  <main class="content question-center" v-loading="loading">
    ...
    <template v-else>
      ...
      <template v-if="statistics">
        ...
        <el-card shadow="never" class="stats-card weak-point-stats">
          <template #header><h3>薄弱知识点</h3></template>
          <el-empty v-if="!statistics.weak_points?.length" description="当前试卷还没有可分析的知识点作答数据" :image-size="48" />
          <div v-else class="weak-point-grid">
            <div v-for="point in statistics.weak_points" :key="point.point" class="weak-point-stat">
              <div><b>{{ point.point }}</b><span>{{ point.wrong_count }}/{{ point.attempts }} 人答错</span></div>
              <el-progress :percentage="point.error_rate" :color="point.error_rate >= 60 ? '#ef4444' : point.error_rate >= 30 ? '#f59e0b' : '#22c55e'" />
            </div>
          </div>
        </el-card>
            ...
            <el-table-column type="expand">
              <template #default="{ row }">
                <el-table v-if="row.wrong_questions.length" :data="row.wrong_questions" size="small">
                  ...
                  <el-table-column label="学生答案" width="150">
                    <template #default="{ row }">{{ questionAnswerLabel(row.response, row.question_type) }}</template>
                  </el-table-column>
                  <el-table-column label="正确答案" width="150">
                    <template #default="{ row }">{{ questionAnswerLabel(row.correct_answer, row.question_type) }}</template>
                  </el-table-column>
                </el-table>
...
.weak-point-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}.weak-point-stat{padding:12px 14px;border:1px solid #e3ece8;border-radius:12px;background:linear-gradient(135deg,#fff,#f7fbfa)}.weak-point-stat>div{display:flex;justify-content:space-between;gap:12px;margin-bottom:8px;color:#294a42}.weak-point-stat span{color:#7d8e88;font-size:12px;white-space:nowrap}.weak-point-stat :deep(.el-progress-bar__outer){background:#edf3f1}@media(max-width:760px){.weak-point-grid{grid-template-columns:1fr}}
