...
async function load(){if(!courseId.value)return;loading.value=true;try{archive.value=(await api.get(`/teacher/courses/${courseId.value}/teaching-archive`,{params:{...(selectedClassId.value?{class_id:selectedClassId.value}:{}),_refresh:Date.now()},headers:{'Cache-Control':'no-cache'}})).data}catch(e:any){ElMessage.error(e.response?.data?.detail||'教学档案加载失败')}finally{loading.value=false}}
