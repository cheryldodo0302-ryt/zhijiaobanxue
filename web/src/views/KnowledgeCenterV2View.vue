...
const resourceJobId=ref(''),resourcePickerVisible=ref(false),resourceDirection=ref<'next'|'previous'>('next'),resourceViewMode=ref<'overview'|'detail'>('overview')
const currentJobIndex=computed(()=>{if(!jobs.value.length)return-1;const index=jobs.value.findIndex((job:any)=>job.document_id===resourceJobId.value);return index>=0?index:0})
const currentJob=computed(()=>currentJobIndex.value>=0?jobs.value[currentJobIndex.value]:null)
const resourceStack=computed(()=>{const center=currentJobIndex.value;if(center<0)return[];return jobs.value.map((job:any,index:number)=>({job,offset:index-center})).filter((item:any)=>item.offset!==0&&Math.abs(item.offset)<=2)})
function selectResource(job:any){if(!job)return;resourceJobId.value=job.document_id;resourcePickerVisible.value=false}
function moveResource(offset:number){const next=currentJobIndex.value+offset;if(next<0||next>=jobs.value.length)return;resourceDirection.value=offset>0?'next':'previous';resourceJobId.value=jobs.value[next].document_id}
function openResourceDetail(job:any=currentJob.value){if(!job)return;selectResource(job);resourceViewMode.value='detail'}
function isJobSelected(job:any){return Boolean(job&&selectedJobs.value.some((item:any)=>item.document_id===job.document_id))}
function toggleJobSelection(job:any,checked:boolean){if(!job)return;const ids=new Set(selectedJobs.value.map((item:any)=>item.document_id));if(checked)ids.add(job.document_id);else ids.delete(job.document_id);selectedJobs.value=jobs.value.filter((item:any)=>ids.has(item.document_id))}
function toggleAllJobs(checked:boolean){selectedJobs.value=checked?[...jobs.value]:[]}
function runJobAction(job:any,action:string){if(action==='review')return openDocument(job);if(action==='confirm')return saveMaterial(job);if(action==='approve')return approveWholeDocument(job);if(action==='retry')return retryParse(job);if(action==='ppt')return rebuildPptTitles(job);if(action==='analyze')return analyze(job);if(action==='delete')return deleteDocument(job)}
function resourceStackStyle(offset:number){return{zIndex:3-Math.abs(offset)}}
...
const treeScopeKey=computed(()=>`${courseId.value}:${outlineMode.value}:${outlineMode.value==='document'?(selectedDoc.value?.document_id||''):selectedMaterialType.value}:${teachingScopeFilter.value}`)
watch(treeScopeKey,async()=>{await nextTick();treeRef.value?.setCheckedKeys([])})
...
const materialPending=(job:any)=>Boolean(job&&(job._saved_material_type!==job.material_type||JSON.stringify(job._saved_tags||[])!==JSON.stringify(job.tags||[])))
async function loadJobs(){if(!courseId.value)return;const id=courseId.value;const[j,r,t]=await Promise.all([api.get(`/teacher/courses/${courseId.value}/ingestion-jobs`),api.get(`/teacher/courses/${courseId.value}/knowledge-workflow`),api.get(`/teacher/courses/${courseId.value}/knowledge-trash`)]);if(courseId.value!==id)return;jobs.value=j.data.map((fresh:any)=>{const old=jobs.value.find((job:any)=>job.document_id===fresh.document_id);const edits=old&&materialPending(old)?{material_type:old.material_type,tags:old.tags}:{};return {...fresh,...edits,_saved_material_type:fresh.material_type,_saved_tags:[...(fresh.tags||[])]}});selectedJobs.value=selectedJobs.value.map((job:any)=>jobs.value.find((fresh:any)=>fresh.document_id===job.document_id)).filter(Boolean);if(!jobs.value.some((job:any)=>job.document_id===resourceJobId.value))resourceJobId.value=jobs.value[0]?.document_id||'';readiness.value=r.data;if(!selectedTrash.value.length)trash.value=t.data;if(selectedDoc.value)selectedDoc.value=jobs.value.find((x:any)=>x.document_id===selectedDoc.value.document_id)||selectedDoc.value}
async function changeCourse(){readiness.value=null;selectedJobs.value=[];selectedTrash.value=[];selectedDoc.value=null;selectedNode.value=null;selectedCandidateId.value='';candidateDraftId.value='';candidateDraft.value='';analysis.value=null;nodes.value=[];relations.value=[];partitions.value=[];teachingLevels.value=[];trash.value=[];candidates.value=[];previewUrl.value='';downloadUrl.value='';resourceJobId.value='';resourcePickerVisible.value=false;resourceViewMode.value='overview';await loadJobs()}
...
async function approveCheckedNodes(){if(outlineMode.value!=='document')return ElMessage.warning('批量批准仅用于文档独立目录');const visible=new Map(visibleNodes.value.filter(x=>x.node_type==='knowledge_point'&&x.status!=='approved').map(x=>[String(x.node_id),x]));const ids=(treeRef.value?.getCheckedKeys(false)||[]).map(String).filter((id:string)=>visible.has(id));if(!ids.length)return ElMessage.warning('请先勾选当前范围内待批准的知识点');try{const result=(await api.post('/teacher/knowledge-nodes/batch-approve',{ids})).data;treeRef.value?.setCheckedKeys([]);ElMessage.success(`已批准 ${result.approved_leaf_count} 个最小知识点，并同步候选审核`);await Promise.all([loadOutline(),loadIngestionMetadata(),loadJobs()])}catch(e){fail(e,'批量批准失败')}}
...
async function mergeChecked(){const checked=(treeRef.value?.getCheckedKeys(false)||[]).map(String);if(checked.some((id:string)=>visibleNodes.value.some(x=>String(x.node_id)===id&&x.node_type!=='knowledge_point')))return ElMessage.warning('合并前请取消章节和节的勾选，仅勾选要合并的知识点');const ids=checked.filter((id:string)=>visibleNodes.value.some(x=>String(x.node_id)===id&&x.node_type==='knowledge_point'));if(ids.length<2)return ElMessage.warning('请勾选至少两个知识点');const{value}=await ElMessageBox.prompt(`将当前勾选的 ${ids.length} 个知识点合并。请输入合并后标题`,'合并知识点',{inputValue:nodes.value.find(x=>x.node_id===ids[0])?.title});await api.post('/teacher/knowledge-nodes/merge',{node_ids:ids,title:value});treeRef.value?.setCheckedKeys([]);await loadOutline()}
...
function checkedDragItems(dragged:any,_siblingMove=false){return[dragged]}
...
<template><main class="content knowledge-center">
  ...
  <el-card shadow="never" class="job-card resource-workspace-card"><template #header><div class="card-header"><div class="card-header-copy"><b>资料、用途与语义分析</b><div class="muted">系统按内容建议用途，教师确认后用于课程知识组织；状态标签会显示文档解析与内容分析的中文进度。</div></div><div class="resource-header-actions"><el-button plain :disabled="!jobs.length" @click="resourcePickerVisible=true">切换资料</el-button><el-button plain :disabled="!jobs.length" @click="resourceViewMode=resourceViewMode==='overview'?'detail':'overview'">{{resourceViewMode==='overview'?'详细信息':'返回资料总览'}}</el-button><el-checkbox v-if="resourceViewMode==='detail'" :model-value="Boolean(jobs.length&&selectedJobs.length===jobs.length)" :indeterminate="selectedJobs.length>0&&selectedJobs.length<jobs.length" @change="toggleAllJobs(Boolean($event))">全选资料</el-checkbox><el-button v-if="resourceViewMode==='detail'" type="danger" plain :disabled="!selectedJobs.length" @click="deleteSelectedDocuments">批量删除（{{selectedJobs.length}}）</el-button></div></div></template>
    <el-empty v-if="!jobs.length" description="当前课程还没有资料"/>
<div v-else-if="resourceViewMode==='overview'" class="resource-overview"><el-button circle :icon="ArrowLeft" :disabled="currentJobIndex<=0" aria-label="上一份资料" @click="moveResource(-1)"/><div class="resource-card-stack"><button v-card-hover="() => moveResource(entry.offset)" type="button" v-for="entry in resourceStack" :key="`stack-${entry.job.document_id}`" :class="['resource-stack-card',entry.offset<0?'resource-stack-prev':'resource-stack-next',`resource-stack-depth-${Math.abs(entry.offset)}`]" :style="resourceStackStyle(entry.offset)" :aria-label="`切换资料：${entry.job.original_name}`" @click="moveResource(entry.offset)"><div class="resource-stack-top"><span>{{entry.offset<0?`前${Math.abs(entry.offset)}张`:`后${entry.offset}张`}}</span><b>{{String(entry.job.original_name||'').split('.').pop()?.toUpperCase()||'FILE'}}</b></div><strong>{{entry.job.original_name}}</strong><small>{{materialLabel(entry.job.material_type)}} · {{documentStatusLabel(entry.job)}}</small><div class="resource-stack-rule"></div><span class="resource-stack-status">{{analysisStatusLabel(entry.job)}}</span></button><Transition :name="`resource-${resourceDirection}`"><article v-if="currentJob" :key="currentJob.document_id" class="resource-overview-card"><div class="resource-overview-heading"><div><span class="toolbar-kicker">当前资料 {{currentJobIndex+1}} / {{jobs.length}}</span><h3>{{currentJob.original_name}}</h3><small>{{materialLabel(currentJob.material_type)}} · {{documentStatusLabel(currentJob)}} · {{analysisStatusLabel(currentJob)}}</small></div><el-tag size="small" type="info">{{String(currentJob.original_name||'').split('.').pop()?.toUpperCase()||'FILE'}}</el-tag></div><div class="resource-overview-status"><el-tag size="small" :type="documentStatusType(currentJob)">{{documentStatusLabel(currentJob)}}</el-tag><el-tag size="small" :type="analysisStatusType(currentJob)">{{analysisStatusLabel(currentJob)}}</el-tag><el-tag size="small" :type="documentWorkflow(currentJob).library_status==='approved'?'success':'info'">{{libraryLabel(currentJob)}}</el-tag></div><div class="resource-overview-summary"><div><span>资料用途</span><strong>{{materialLabel(currentJob.material_type)}}</strong></div><div><span>学生版本</span><strong>{{documentWorkflow(currentJob).student_version?`v${documentWorkflow(currentJob).student_version}`:'尚未发布'}}</strong></div><div><span>学生端原文件</span><strong>{{currentJob.student_file_visible?'已允许查看':'未开放查看'}}</strong></div></div><div class="resource-overview-actions"><el-button type="primary" @click="openResourceDetail(currentJob)">查看详细信息</el-button><el-button plain @click="openDocument(currentJob)">审查文档</el-button></div></article></Transition></div><el-button circle :icon="ArrowRight" :disabled="currentJobIndex>=jobs.length-1" aria-label="下一份资料" @click="moveResource(1)"/></div>
    <div v-else class="resource-detail-layout">
      <aside class="resource-detail-panel">
        <div class="resource-detail-heading"><span class="toolbar-kicker">当前资料</span><span class="resource-counter">{{currentJobIndex+1}} / {{jobs.length}}</span></div>
        <div class="resource-file-heading"><span class="resource-file-icon"><el-icon><Document /></el-icon></span><div><strong>{{currentJob.original_name}}</strong><small>课程资料 · {{materialLabel(currentJob.material_type)}}</small></div></div>
        <div class="resource-status-pills"><el-tag size="small" :type="documentStatusType(currentJob)">{{documentStatusLabel(currentJob)}}</el-tag><el-tag size="small" :type="analysisStatusType(currentJob)">{{analysisStatusLabel(currentJob)}}</el-tag></div>
        <div class="resource-form-grid"><div><label>资料用途</label><el-select v-model="currentJob.material_type" size="small"><el-option v-for="item in materialTypes" :key="item[0]" :label="item[1]" :value="item[0]"/></el-select><small class="classification-hint">{{materialPending(currentJob)?'修改待保存；学生端仍显示已保存分类':currentJob.classification_status==='confirmed'?'教师已确认':`系统建议：${materialLabel(currentJob.suggested_material_type)}`}}</small></div><div><label>分类标签</label><el-select v-model="currentJob.tags" multiple filterable allow-create default-first-option size="small" placeholder="输入标签并回车"/></div></div>
        <div class="resource-facts"><div><span>知识库状态</span><strong>{{libraryLabel(currentJob)}}</strong></div><div><span>学生版本</span><strong>{{documentWorkflow(currentJob).student_version?`v${documentWorkflow(currentJob).student_version} 含本资料`:'尚未发布'}}</strong></div></div>
        <div class="resource-student-visibility"><div><strong>学生查看原文件</strong><small>随知识发布后可预览</small></div><el-switch :model-value="Boolean(currentJob.student_file_visible)" :loading="visibilitySaving.includes(currentJob.document_id)" :aria-label="`允许学生预览 ${currentJob.original_name}`" @change="setStudentFileVisible(currentJob,Boolean($event))"/></div>
        <div class="resource-primary-actions"><el-button type="primary" @click="openDocument(currentJob)">审查文档</el-button><el-button type="success" plain @click="saveMaterial(currentJob)">确认分类</el-button><el-button type="warning" plain :disabled="completingReview||!documentWorkflow(currentJob).can_approve" @click="approveWholeDocument(currentJob)">批准到知识库</el-button></div>
        <div class="resource-switcher"><el-button circle :icon="ArrowLeft" :disabled="currentJobIndex<=0" aria-label="上一份资料" @click="moveResource(-1)"/><span>切换资料</span><el-button circle :icon="ArrowRight" :disabled="currentJobIndex>=jobs.length-1" aria-label="下一份资料" @click="moveResource(1)"/></div>
      </aside>
      <section class="resource-list-panel"><div class="resource-list-heading"><div><strong>详细信息列表</strong><small>点击资料行可更新左侧详情；每行的更多菜单保留原有处理功能。</small></div><span v-if="selectedJobs.length" class="resource-selection-count">已选择 {{selectedJobs.length}} 项</span></div><div class="resource-list-table"><div class="resource-list-header"><span>选择</span><span>文件名</span><span>资料用途</span><span>文档解析</span><span>内容分析</span><span>知识库状态</span><span>学生端原文件</span><span>操作</span></div><div class="resource-list-body"><div v-for="job in jobs" :key="job.document_id" class="resource-list-row" :class="{selected:isJobSelected(job),active:job.document_id===currentJob.document_id}" @click="selectResource(job)"><el-checkbox :model-value="isJobSelected(job)" aria-label="选择资料" @click.stop @change="toggleJobSelection(job,Boolean($event))"/><div class="resource-list-file"><span class="resource-file-icon small"><el-icon><Document /></el-icon></span><strong :title="job.original_name">{{job.original_name}}</strong></div><div class="resource-list-purpose"><span>{{materialLabel(job.material_type)}}</span><small>{{job.tags?.length?job.tags.join('、'):'未添加标签'}}</small></div><el-tag size="small" :type="documentStatusType(job)">{{documentStatusLabel(job)}}</el-tag><el-tag size="small" :type="analysisStatusType(job)">{{analysisStatusLabel(job)}}</el-tag><el-tag size="small" :type="documentWorkflow(job).library_status==='approved'?'success':'info'">{{libraryLabel(job)}}</el-tag><el-switch :model-value="Boolean(job.student_file_visible)" :loading="visibilitySaving.includes(job.document_id)" :aria-label="`允许学生预览 ${job.original_name}`" @click.stop @change="setStudentFileVisible(job,Boolean($event))"/><el-dropdown trigger="click" @command="runJobAction(job,String($event))" @click.stop><el-button link>更多<el-icon><ArrowDown /></el-icon></el-button><template #dropdown><el-dropdown-menu><el-dropdown-item command="review">审查</el-dropdown-item><el-dropdown-item command="confirm">确认分类</el-dropdown-item><el-dropdown-item command="approve" :disabled="completingReview||!documentWorkflow(job).can_approve">批准到知识库</el-dropdown-item><el-dropdown-item v-if="job.status==='failed'||(Number(job.document_block_count||0)===0&&job.status==='review_required')" command="retry">重新解析</el-dropdown-item><el-dropdown-item v-if="String(job.original_name||'').toLowerCase().endsWith('.pptx')" command="ppt" :disabled="['queued','running'].includes(job.status)">重建PPT标题</el-dropdown-item><el-dropdown-item command="analyze" :disabled="!['ready','review_required'].includes(job.status)||['queued','running'].includes(job.analysis_status)||Number(job.document_block_count||0)===0">重新分析</el-dropdown-item><el-dropdown-item command="delete" divided>删除</el-dropdown-item></el-dropdown-menu></template></el-dropdown></div></div></div></section>
    </div>
  </el-card>
  <el-dialog v-model="resourcePickerVisible" title="选择已上传资料" width="min(620px, calc(100% - 32px))" class="knowledge-dialog resource-picker-dialog" destroy-on-close><div class="resource-picker-list"><button v-for="job in jobs" :key="`picker-${job.document_id}`" type="button" class="resource-picker-item" :class="{active:job.document_id===currentJob?.document_id}" @click="selectResource(job)"><span class="resource-file-icon small"><el-icon><Document /></el-icon></span><span><strong>{{job.original_name}}</strong><small>{{materialLabel(job.material_type)}} · {{documentStatusLabel(job)}} · {{analysisStatusLabel(job)}}</small></span><el-icon v-if="job.document_id===currentJob?.document_id"><Document /></el-icon></button></div></el-dialog>
  ...
  <section v-if="selectedDoc&&reviewMode==='candidates'" ref="reviewWorkspace" class="structured-review candidate-review">
    ...
    <div class="source-review">
      <div class="page-selector"><el-button v-if="previewKind==='pdf'" :disabled="page<=1" @click="go(page-1)">上一页</el-button><el-input-number v-if="previewKind==='pdf'" v-model="pageInput" :controls="false" :min="1" :max="totalPages" @keyup.enter="go()"/><el-button v-if="previewKind==='pdf'" @click="go()">跳转</el-button><strong v-if="previewKind==='pdf'">{{page}} / {{totalPages}}</strong><el-button v-if="previewKind==='pdf'" :disabled="page>=totalPages" @click="go(page+1)">下一页</el-button><el-tag v-else-if="previewKind==='pptx'" type="success">浏览器内 PPTX 预览</el-tag><el-tag v-else-if="previewKind==='docx'" type="success">浏览器内 Word 预览</el-tag><el-tag v-else-if="previewKind==='spreadsheet'" type="success">浏览器内表格预览</el-tag><el-button tag="a" :href="downloadUrl" target="_blank">下载原文件</el-button></div><iframe v-if="previewKind==='pdf'&&pdfUrl" :key="pdfUrl" :src="pdfUrl" title="原文件对照"/><iframe v-else-if="['docx','spreadsheet'].includes(previewKind)&&previewText" sandbox="" :srcdoc="previewText" title="资料原文件对照"/><div v-else-if="previewKind==='pptx'" ref="pptxHost" class="pptx-preview-host"/><div v-else-if="['markdown','text'].includes(previewKind)" class="rendered-markdown source-markdown" v-html="markdownHtml"/><el-result v-else icon="warning" title="预览不可用" :sub-title="previewError||'请下载原文件查看'"/>
    </div>
...
.resource-workspace-card{margin-top:16px}.resource-workspace-card>.el-card__body{padding:0}.resource-header-actions{display:flex;align-items:center;justify-content:flex-end;gap:10px;flex-wrap:wrap}.resource-header-actions .el-checkbox{margin-right:4px}.resource-overview{display:grid;grid-template-columns:48px minmax(0,1fr) 48px;align-items:center;gap:14px;min-height:390px;padding:28px 36px;background:#f4f5ed}.resource-overview>.el-button{margin:0;border-color:#dce1d4;color:#294b3c;background:#fcfcf8}.resource-card-stack{position:relative;display:grid;place-items:center;min-height:334px;perspective:900px}.resource-stack-card{grid-area:1 / 1;width:min(780px,calc(100% - 44px));min-height:246px;padding:26px 30px;border:1px solid #dce1d4;border-radius:12px;background:#fcfcf8;color:#56634d;overflow:hidden;box-shadow:0 10px 24px rgba(41,75,60,.06);z-index:1;pointer-events:none}.resource-stack-card span,.resource-stack-card small{display:block;font-size:12px}.resource-stack-card strong{display:block;margin:14px 0 8px;color:#294b3c;font-size:17px;line-height:1.45;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.resource-stack-card small{color:#56634d}.resource-stack-prev{transform:translate3d(-25px,16px,-24px) rotate(-2.2deg) scale(.95)}.resource-stack-next{transform:translate3d(25px,16px,-24px) rotate(2.2deg) scale(.95)}.resource-overview-card{grid-area:1 / 1;position:relative;z-index:3;width:min(780px,100%);margin:0 auto;padding:26px 30px;border:1px solid #dce1d4;border-radius:12px;background:#fcfcf8;box-shadow:0 12px 30px rgba(41,75,60,.08)}.resource-overview-heading{display:flex;align-items:flex-start;justify-content:space-between;gap:20px;padding-bottom:18px;border-bottom:1px solid #dce1d4}.resource-overview-heading>div{display:grid;gap:6px;min-width:0}.resource-overview-heading h3{margin:0;color:#294b3c;font-size:22px;line-height:1.4;overflow-wrap:anywhere}.resource-overview-heading small{color:#56634d;font-size:12px}.resource-overview-status{display:flex;flex-wrap:wrap;gap:8px;margin:18px 0}.resource-overview-summary{display:grid;grid-template-columns:repeat(3,1fr);gap:10px}.resource-overview-summary>div{display:grid;gap:6px;min-width:0;padding:12px;border-radius:8px;background:#edf0e7}.resource-overview-summary span{color:#56634d;font-size:11px}.resource-overview-summary strong{color:#294b3c;font-size:13px;overflow-wrap:anywhere}.resource-overview-actions{display:flex;justify-content:center;gap:10px;margin-top:24px}.resource-overview-actions .el-button{margin:0}.resource-detail-layout{display:grid;grid-template-columns:minmax(300px,.78fr) minmax(0,1.72fr);gap:16px;padding:16px;background:#f4f5ed}.resource-detail-panel,.resource-list-panel{min-width:0;border:1px solid #dce1d4;border-radius:10px;background:#fcfcf8}.resource-detail-panel{display:grid;align-content:start;gap:14px;padding:18px}.resource-detail-heading,.resource-list-heading,.resource-student-visibility,.resource-switcher{display:flex;align-items:center;justify-content:space-between;gap:12px}.resource-counter,.resource-selection-count{font-size:12px;color:#56634d;font-variant-numeric:tabular-nums}.resource-file-heading{display:flex;align-items:flex-start;gap:12px;min-width:0;padding-bottom:12px;border-bottom:1px solid #dce1d4}.resource-file-heading>div{display:grid;gap:5px;min-width:0}.resource-file-heading strong{color:#294b3c;font-size:16px;line-height:1.45;overflow-wrap:anywhere}.resource-file-heading small{color:#56634d;font-size:12px}.resource-file-icon{display:grid;place-items:center;flex:0 0 42px;width:42px;height:42px;border-radius:10px;background:#e4e9da;color:#294b3c}.resource-file-icon.small{flex-basis:28px;width:28px;height:28px;border-radius:7px}.resource-file-icon .el-icon{font-size:21px}.resource-file-icon.small .el-icon{font-size:16px}.resource-status-pills{display:flex;flex-wrap:wrap;gap:7px}.resource-form-grid{display:grid;gap:12px}.resource-form-grid>div{display:grid;gap:6px;min-width:0}.resource-form-grid label{font-size:12px;font-weight:600;color:#294b3c}.resource-form-grid :deep(.el-select){width:100%}.resource-form-grid :deep(.el-select__tags){max-width:calc(100% - 34px)}.resource-facts{display:grid;grid-template-columns:1fr 1fr;gap:8px}.resource-facts>div{display:grid;gap:5px;min-width:0;padding:10px;border-radius:8px;background:#edf0e7}.resource-facts span,.resource-student-visibility small{font-size:11px;color:#56634d}.resource-facts strong{font-size:12px;color:#294b3c;line-height:1.45;overflow-wrap:anywhere}.resource-student-visibility{padding:11px 0;border-top:1px solid #dce1d4;border-bottom:1px solid #dce1d4}.resource-student-visibility>div{display:grid;gap:4px}.resource-student-visibility strong{font-size:13px;color:#294b3c}.resource-primary-actions{display:grid;grid-template-columns:1fr 1fr;gap:8px}.resource-primary-actions .el-button{width:100%;margin:0}.resource-primary-actions .el-button:last-child{grid-column:1/-1}.resource-switcher{justify-content:center;padding-top:2px;color:#56634d;font-size:12px}.resource-switcher .el-button{margin:0;border-color:#dce1d4;color:#294b3c;background:#fcfcf8}.resource-list-panel{display:flex;flex-direction:column;overflow:hidden}.resource-list-heading{padding:18px 18px 14px;border-bottom:1px solid #dce1d4}.resource-list-heading>div{display:grid;gap:5px;min-width:0}.resource-list-heading strong{font-size:16px;color:#294b3c}.resource-list-heading small{color:#56634d;font-size:12px;line-height:1.5}.resource-list-table{min-width:0}.resource-list-header,.resource-list-row{display:grid;grid-template-columns:30px minmax(180px,2fr) minmax(90px,.85fr) minmax(110px,1fr) minmax(110px,1fr) minmax(110px,1fr) 90px 58px;gap:10px;align-items:center}.resource-list-header{padding:11px 14px;background:#edf0e7;color:#56634d;font-size:12px;font-weight:650}.resource-list-body{max-height:min(58vh,580px);overflow-y:auto;overflow-x:hidden}.resource-list-row{min-height:72px;padding:10px 14px;border-top:1px solid #dce1d4;color:#294b3c;cursor:pointer;transition:background-color .16s ease,box-shadow .16s ease}.resource-list-row:hover,.resource-list-row.active{background:#f1f5ed}.resource-list-row.selected{box-shadow:inset 3px 0 #5d7350;background:#edf0e7}.resource-list-row>.el-checkbox{margin:0}.resource-list-file{display:flex;align-items:center;gap:8px;min-width:0}.resource-list-file strong{min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:12px;color:#294b3c}.resource-list-purpose{display:grid;gap:4px;min-width:0}.resource-list-purpose span{font-size:12px;color:#294b3c}.resource-list-purpose small{overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:#56634d;font-size:11px}.resource-list-row>.el-tag{justify-self:start;max-width:100%;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.resource-list-row>.el-switch{justify-self:start}.resource-list-row>.el-dropdown{justify-self:end}.resource-list-row>.el-dropdown .el-button{margin:0;padding:4px 0;color:#294b3c}.resource-picker-dialog :deep(.el-dialog__body){padding:12px 18px 18px}.resource-picker-list{display:grid;gap:6px;max-height:56vh;overflow:auto}.resource-picker-item{display:grid;grid-template-columns:28px minmax(0,1fr) auto;align-items:center;gap:10px;width:100%;padding:10px;border:1px solid #dce1d4;border-radius:8px;background:#fcfcf8;color:#294b3c;text-align:left;cursor:pointer}.resource-picker-item:hover,.resource-picker-item.active{border-color:#5d7350;background:#edf0e7}.resource-picker-item>span:nth-child(2){display:grid;gap:4px;min-width:0}.resource-picker-item strong{overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:13px}.resource-picker-item small{color:#56634d;font-size:11px}.resource-picker-item>.el-icon{color:#5d7350}
@media(prefers-reduced-motion:no-preference){.resource-list-row{transition:background-color .16s ease,box-shadow .16s ease}.resource-stack-card{transition:transform .58s cubic-bezier(.22,1,.36,1),box-shadow .25s ease;will-change:transform}.resource-next-enter-active,.resource-next-leave-active,.resource-previous-enter-active,.resource-previous-leave-active{transition:transform .58s cubic-bezier(.22,1,.36,1),opacity .38s ease}.resource-next-enter-from,.resource-previous-leave-to{opacity:0;transform:translate3d(86px,28px,0) rotate(4deg) scale(.93)}.resource-next-leave-to,.resource-previous-enter-from{opacity:0;transform:translate3d(-72px,-14px,0) rotate(-4deg) scale(.96)}}
@media(prefers-reduced-motion:no-preference){.resource-list-row{transition:background-color .16s ease,box-shadow .16s ease}}
@container knowledge-center (max-width:1180px){.resource-detail-layout{grid-template-columns:minmax(270px,.8fr) minmax(0,1.2fr);gap:12px;padding:12px}.resource-list-header,.resource-list-row{grid-template-columns:28px minmax(150px,1.7fr) minmax(76px,.75fr) repeat(3,minmax(86px,1fr)) 72px 52px;gap:7px;padding-inline:10px}.resource-list-heading{padding:14px}.resource-list-header{font-size:11px}.resource-list-row{min-height:68px}.resource-list-row>.el-tag{font-size:11px}}
@container knowledge-center (max-width:820px){.resource-overview{grid-template-columns:36px minmax(0,1fr) 36px;gap:8px;padding:18px 12px}.resource-overview-card{padding:20px}.resource-detail-layout{grid-template-columns:1fr}.resource-list-body{max-height:520px}.resource-detail-panel{padding:16px}}
@media(prefers-reduced-motion:no-preference){.resource-next-enter-active,.resource-next-leave-active,.resource-previous-enter-active,.resource-previous-leave-active{transition:transform .32s cubic-bezier(.16,1,.3,1),opacity .22s ease}.resource-next-enter-from,.resource-previous-leave-to{opacity:0;transform:translate3d(46px,18px,0) rotate(1.5deg) scale(.965)}.resource-next-leave-to,.resource-previous-enter-from{opacity:0;transform:translate3d(-36px,-8px,0) rotate(-1.5deg) scale(.985)}}
@media(prefers-reduced-motion:reduce){.resource-stack-card,.resource-next-enter-active,.resource-next-leave-active,.resource-previous-enter-active,.resource-previous-leave-active{transition:none}}
@container knowledge-center (max-width:620px){.resource-header-actions{justify-content:flex-start}.resource-overview{grid-template-columns:1fr;gap:12px}.resource-overview>.el-button{justify-self:center}.resource-overview>.el-button:first-child{grid-row:3}.resource-card-stack{min-height:316px}.resource-stack-card{width:calc(100% - 20px);padding:20px}.resource-overview-card{padding:18px}.resource-overview-heading h3{font-size:18px}.resource-overview-summary{grid-template-columns:1fr}.resource-overview-actions{align-items:stretch;flex-direction:column}.resource-overview-actions .el-button{width:100%}.resource-detail-layout{padding:10px}.resource-list-header{display:none}.resource-list-row{grid-template-columns:28px minmax(0,1fr) auto;gap:8px;padding:10px}.resource-list-purpose,.resource-list-row>.el-tag,.resource-list-row>.el-switch{display:none}.resource-list-file strong{white-space:normal;line-height:1.45}.resource-list-row>.el-dropdown{grid-column:3;grid-row:1}.resource-list-heading{align-items:flex-start;flex-direction:column}.resource-facts{grid-template-columns:1fr}.resource-primary-actions{grid-template-columns:1fr}.resource-primary-actions .el-button:last-child{grid-column:auto}}
/* Keep the fan inside its own hit-testing area; controls sit above it. */
.resource-overview{position:relative;isolation:isolate;overflow:hidden}
.resource-overview>.el-button{position:relative;z-index:10}
.resource-card-stack{min-width:0;min-height:430px;isolation:isolate;perspective:none;grid-template-columns:minmax(0,1fr)}
.resource-overview-card{width:58%;min-width:0;z-index:4;box-sizing:border-box}
.resource-stack-card{display:block;width:58%;min-width:0;min-height:300px;box-sizing:border-box;font:inherit;text-align:left;cursor:pointer;pointer-events:auto;transform-origin:center 80%}
.resource-stack-card:focus-visible{outline:3px solid #294b3c;outline-offset:3px}
.resource-stack-card.resource-stack-prev.resource-stack-depth-1{transform:translate(-35%,14px) rotate(-7deg) scale(.9)}
.resource-stack-card.resource-stack-prev.resource-stack-depth-2{transform:translate(-65%,30px) rotate(-13deg) scale(.8)}
.resource-stack-card.resource-stack-next.resource-stack-depth-1{transform:translate(35%,14px) rotate(7deg) scale(.9)}
.resource-stack-card.resource-stack-next.resource-stack-depth-2{transform:translate(65%,30px) rotate(13deg) scale(.8)}
.resource-stack-top{display:flex;justify-content:space-between;gap:12px}
.resource-stack-top b{color:#5d7350;font-size:12px}
.resource-stack-card strong{white-space:normal;overflow-wrap:anywhere;margin:30px 0 12px;font-size:18px}
.resource-stack-rule{height:1px;margin:24px 0 14px;background:#dce1d4}
.resource-next-leave-active,.resource-previous-leave-active{pointer-events:none}
@media(prefers-reduced-motion:no-preference){
.resource-stack-card{transition:transform .6s ease-in-out}
.resource-next-enter-active,.resource-next-leave-active,.resource-previous-enter-active,.resource-previous-leave-active{transition:transform .6s ease-in-out,opacity .6s ease-in-out}
}
@container knowledge-center (max-width:820px){.resource-overview-card{width:64%;padding:18px}.resource-overview-summary{grid-template-columns:1fr}.resource-overview-actions{flex-wrap:wrap}}
@container knowledge-center (max-width:620px){.resource-overview{grid-template-columns:36px minmax(0,1fr) 36px;padding:16px 4px;gap:4px}.resource-overview>.el-button:first-child{grid-row:auto}.resource-overview-card{width:100%}.resource-stack-card{display:none}.resource-card-stack{min-height:316px}}
@media (hover:hover) and (pointer:fine) {
  .resource-stack-card.card-hovered { z-index:6!important; box-shadow:0 24px 44px rgba(41,75,60,.2); }
  .resource-stack-prev.resource-stack-depth-1.card-hovered { transform:translate3d(-35%,2px,26px) rotate(-7deg) scale(.98) rotateX(var(--hover-rotate-x,0deg)) rotateY(var(--hover-rotate-y,0deg)); }
  .resource-stack-prev.resource-stack-depth-2.card-hovered { transform:translate3d(-65%,14px,18px) rotate(-13deg) scale(.88) rotateX(var(--hover-rotate-x,0deg)) rotateY(var(--hover-rotate-y,0deg)); }
  .resource-stack-next.resource-stack-depth-1.card-hovered { transform:translate3d(35%,2px,26px) rotate(7deg) scale(.98) rotateX(var(--hover-rotate-x,0deg)) rotateY(var(--hover-rotate-y,0deg)); }
  .resource-stack-next.resource-stack-depth-2.card-hovered { transform:translate3d(65%,14px,18px) rotate(13deg) scale(.88) rotateX(var(--hover-rotate-x,0deg)) rotateY(var(--hover-rotate-y,0deg)); }
  .resource-overview-card:hover { transform:translate3d(0,-4px,0) scale(1.025); }
}
@media (prefers-reduced-motion:no-preference) {
  .resource-stack-card,.resource-overview-card { transition:transform .72s cubic-bezier(.22,1,.36,1),opacity .72s ease,box-shadow .24s ease; will-change:transform; }
}
@media (prefers-reduced-motion:reduce) {
  .resource-stack-card,.resource-overview-card { transition:none; }
  .resource-stack-card.card-hovered,.resource-overview-card:hover { transform:none; }
}
@media (prefers-reduced-motion:no-preference) {
  .resource-card-stack{perspective:1200px;transform-style:preserve-3d}
  .resource-next-enter-active,.resource-next-leave-active,.resource-previous-enter-active,.resource-previous-leave-active{transition:transform .72s cubic-bezier(.22,1,.36,1),opacity .48s ease,box-shadow .24s ease}
  .resource-next-enter-from{opacity:.5;transform:translate3d(35%,18px,20px) rotateY(-14deg) rotateZ(7deg) scale(.9)}
  .resource-next-leave-to{opacity:.08;transform:translate3d(-35%,18px,-10px) rotateY(14deg) rotateZ(-7deg) scale(.9)}
  .resource-previous-enter-from{opacity:.5;transform:translate3d(-35%,18px,20px) rotateY(14deg) rotateZ(-7deg) scale(.9)}
  .resource-previous-leave-to{opacity:.08;transform:translate3d(35%,18px,-10px) rotateY(-14deg) rotateZ(7deg) scale(.9)}
}
</style>
